...
export const updateEventStatus = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `Etkinlik durumu güncelleme isteği: eventId=${id}, status=${status}`
    );
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (userError) {
          logger.error(
            `Error finding user by ID: ${JSON.stringify(userError, null, 2)}`
          );
        }
        ...
        if (!userData) {
          ...
          logger.info(
            `Kullanıcı veritabanında bulundu: ${JSON.stringify(userData)}`
          );
        }
      ...
      logger.info("Veri tipleri:", {
        ...
        location_lat:
          typeof location_lat === "string"
            ...
            : typeof location_lat,
        location_long:
          typeof location_long === "string"
            ? `string: ${location_long}`
            : typeof location_long,
        max_participants:
          typeof max_participants === "string"
            ? `string: ${max_participants}`
            : typeof max_participants,
      });
      ...
      try {
        ...
        logger.error(
          `Tarih doğrulama hatası: ${
            dateError instanceof Error ? dateError.message : "Bilinmeyen hata"
          }`
        );
      }
      ...
      logger.info(
        `Supabase yapılandırma kontrolü: URL=${
          supabaseURLExists ? "Mevcut" : "Eksik"
        }, ServiceKey=${supabaseKeyExists ? "Mevcut" : "Eksik"}`
      );
      ...
      logger.info(
        `Servise gönderilecek veri: ${JSON.stringify(newEventData, null, 2)}`
      );
      ...
      try {
        ...
        logger.error(
          ...
          {
            stack:
              serviceError instanceof Error ? serviceError.stack : "Stack yok",
          }
        );
        ...
        if (
          serviceError instanceof Error &&
          serviceError.message.includes("23503")
        ) {
        ...
        if (
          serviceError instanceof Error &&
          serviceError.message.includes("22P02")
        ) {
...
export const getAllEvents = async (req: Request, res: Response) => {
  try {
    ...
    // Extract status filter from query parameters and convert to proper format
    let statusFilter: string | string[] | undefined;

    if (req.query.status) {
      // Handle both array and single string cases
      if (Array.isArray(req.query.status)) {
        // If an array is passed (e.g., ?status=ACTIVE&status=REJECTED)
        statusFilter = req.query.status as string[];
      } else {
        // If a single value is passed (e.g., ?status=PENDING)
        statusFilter = req.query.status as string;
      }
...
export const getTodayEvents = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(
      `Kullanıcı durumu: ${userId ? "Giriş yapmış" : "Giriş yapmamış"}`
    );
